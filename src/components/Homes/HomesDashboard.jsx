...
import { DashboardHomes } from "../../data";
...
const HomesDashboard = () => {
  return (
    <div className="mx-auto container mt-24 pb-24 w-[75%] ">
        ...
        <div className="flex flex-col sm:flex-row mt-6">
          {DashboardHomes.map((homes) => (
            <div
              ...
              key={homes.id}
            >
              <NavLink to={`/homes/homesMyInvestments/${homes.id.toString()}`}>
      ...
      <div className="flex flex-col mt-24 sm:mt-14 ">
        <h2 className=" text-[30px] leading-[38px] tracking-tighter">
          Recommended Homes
        </h2>
        <div className="flex flex-col  sm:flex-row mt-2 sm:mt-4">
          {DashboardHomes.map((homes) => (
            <div
              className={`mt-4 w-full flex flex-col max-sm:mt-8 sm:w-[23%] ${
                homes.id >= 1 && homes.id <= 3 ? "sm:ml-4 " : "ml-0"
              }`}
              key={homes.id}
            >
              <NavLink to={`/homes/homesInvestments/${homes.id.toString()}`}>
                <img src={homes.img} className="w-full" alt="" />
                <div className="mt-6">
                  <h3 className="text-[16px] leading-[25px] tracking-tighter">
                    {homes.name}
